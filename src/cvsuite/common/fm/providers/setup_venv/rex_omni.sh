...
pip install "transformers==4.51.3" "qwen_vl_utils==0.0.14" accelerate pillow numpy pyyaml --no-cache-dir

# Install rex_omni without deps so it skips vllm/gradio/flash-attn and reuses the installed torch stack.
pip install --no-deps --no-cache-dir git+https://github.com/IDEA-Research/Rex-Omni.git
